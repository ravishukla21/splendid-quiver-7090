...
export default function MenShirtSidebar() {
  const [brandSearch, setBrandSearch] = React.useState([]);
  const [PriceUnder, setPriceUnder] = React.useState([]);
  const [sortBy, setSortBy] = React.useState("");

  const dispatch = useDispatch();
  const stores = useSelector((store) => store.MenShirtReducer.MenShirt);

  // console.log(stores);

  React.useEffect(() => {
    dispatch(getManShirtDataAction(brandSearch, PriceUnder, sortBy));
